...
class LEDMatrix(SecureArduino):
    ...
    def set_message(self, message, mode=None, speed=None):
        ...
        if mode is None and len(message) > 1:
            ...
            mode = ANIMATION_MODE
        self.send(SET_MATRIX_MESSAGE_OPCODE, BYTE(
            self.matrix_id), BYTE(mode), STRING(message))
    ...
    def set_default_message(self, message, mode, speed):
        ...
        self.send(SET_EEPROM_DEFAULT_MESSAGE_OPCODE, BYTE(
            self.matrix_id), INT(speed), BYTE(mode), STRING(message))
    ...
    def upload_char_pattern(self, char, pattern):
        ...
        self.send(SET_EEPROM_CHAR_LEDMATRIX_OPCODE, CHAR(
            ord(char)), *lines, BYTE(pattern.width))
    ...
    def upload_charset(self, charset):
        ...
        for i in range(16):
            for j in range(6):
                x0, y0 = 8 * i, 8 * j
                x1, y1 = x0 + 8, y0 + 8
                while charset.getpixel((x1 - 1, y0)) not in (0, 255):
                    x1 -= 1
                while charset.getpixel((x0, y1 - 1)) not in (0, 255):
                    y1 -= 1
